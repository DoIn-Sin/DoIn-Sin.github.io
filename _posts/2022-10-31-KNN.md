...
- <img src="https://latex.codecogs.com/svg.latex?\Large&space;\left(\displaystyle\sum_{i=1}^{n} \vert {x_i - y_i} \vert^p \right)^{1/p}, (i = 각 벡터의 i^{번째} 요소, x, y = 거리를 계산할 데이터 객체, p = 1 또는 2의 정수)" title="\left(\displaystyle\sum_{i=1}^{n} \vert {x_i - y_i} \vert^p \right)^{1/p}, (i = 각 벡터의 i^{번째} 요소, x, y = 거리를 계산할 데이터 객체, p = 1 또는 2의 정수)" />
